...
case class ContainerDbInfo(activationId: String, 
                          ...
                          workingTimestamp: Option[Double], // 容器进入 working 状态的时间戳，用于计算容器已经进行SQL的时间
                          predictTime: Option[Double]       // 预测持续时间，以秒为单位
                          )

// 定义 JsonProtocol
object ContainerDbInfoJsonProtocol extends DefaultJsonProtocol {
  // 定义 ContainerDbInfo 的 JsonFormat
  implicit val containerDbInfoFormat: RootJsonFormat[ContainerDbInfo] = jsonFormat8(ContainerDbInfo)
}
...
object ContainerDbInfoManager {
  ...
  def readDbInfo()(implicit logging: Logging): Map[String, ContainerDbInfo] = {
    ...
    if (jsonStr.trim.isEmpty) {
      logging.warn(this, s"$dbInfoFilePath is empty!")
      return Map.empty[String, ContainerDbInfo]
    }
  ...
  def createDbInfo(creationId: String, dbFile: String, tables: List[String])(implicit logging: Logging): Unit = {
    ...
    val state = "loading"
    val workingTimestamp: Option[Double] = None // 默认没有时间戳
    val predictTime: Option[Double] = None // 默认没有预测时间
    val updatedInfo = dbInfo + (creationId -> ContainerDbInfo(activationId, creationId, containerId, dbFile, tables, state, workingTimestamp, predictTime))
  ...
  // 当某个容器被分配时，将containerId对应的容器态置为working
  def updateStateToLoading(containerId: String)(implicit logging: Logging): Unit = {
    val dbInfo = readDbInfo()
    val updatedInfo = dbInfo.get(containerId).map { info =>
      info.copy(state = "loading", workingTimestamp = None) // 设置为 "loading" 状态
    }
    val newDbInfo = dbInfo + (containerId -> updatedInfo.get)
    writeDbInfo(newDbInfo)
  }

  def updateStateToWarm(containerId: String)(implicit logging: Logging): Unit = {
    logging.info(this, s"正在更新容器状态为warm, containerId: $containerId")
    val dbInfo = readDbInfo()
    logging.info(this, s"now dbInfo: $dbInfo")
    val updatedInfo = dbInfo.get(containerId).map { info =>
      info.copy(state = "warm", workingTimestamp = None) // 重置状态并清除时间戳
    }
    val newDbInfo = dbInfo + (containerId -> updatedInfo.get)
    writeDbInfo(newDbInfo)
  }
