...
object StartInfoManager {
  ...
  // 定义文件路径为对象的成员变量
  val StartInfoFilePath: String = "/db/start_info.json"

  // 读取 StartInfo 文件内容的函数
  def readStartInfo(): JsArray = {
  ...
  // 写入 StartInfo 到文件的函数
  def writeStartInfo(jsonArray: JsArray): Unit = {
    val writer = new PrintWriter(StartInfoFilePath)
    try {
  ...
  // 针对热启动情况，更新StartInfo 文件
  def updateContainerIdByCreationId(creationId: String, newContainerId: String)(implicit logging: Logging): Unit = {
    // 1. 读取现有的 StartInfo 文件
    val StartInfoArray = readStartInfo()

    // 日志记录
    logging.info(this, s"StartInfoArray: $StartInfoArray")
    ...
    val updatedArray = StartInfoArray.elements.map {
      case obj: JsObject if obj.fields.get("creationId").contains(JsString(creationId)) =>
        ...
        dbInfo.get(newContainerId) match {
          case Some(containerDbInfo) =>
            logging.info(this ,s"find containerDbInfo: $containerDbInfo")
            // 获取 dbName
            val dbName = containerDbInfo.dbName
            logging.info(this, s"dbName is $dbName")
          ...
          case None =>
            ...
            logging.error(this, s"only find containerId in dbInfo, no containerId or dbName")
    ...
    // 4. 将更新后的数组写回文件
    writeStartInfo(JsArray(updatedArray.toVector))
  ...
  def updateContainerIdByActivationId(activationId: String, newContainerId: String)(implicit logging: Logging): Unit = {
    val StartInfoArray = readStartInfo() // 读取的是JsArray

    val updatedArray = StartInfoArray.elements.map { element =>
    ...
    writeStartInfo(JsArray(updatedArray.toVector)) // 写入更新后的数组
    logging.info(this, s"update containerId of ActivationId: $ActivationId ")
  }

  // 创建新的配对并添加到 StartInfo 文件中的函数
  def createStartInfo(creationId: String, activationId: String, containerId: String, dbName: String, startMode: String): Unit = {
    // 1. 创建新的配对对象
    val newStartInfo = JsObject(
    ...
    // 2. 读取现有的 StartInfo 数组
    val StartInfoArray = readStartInfo()

    // 3. 将新的配对对象添加到数组中
    val updatedArray = StartInfoArray.elements :+ newStartInfo
...
object ContainerManager {
  ...
  protected[container] def filterWarmedCreations(warmedContainers: Set[String],
    ...
    val warmedApplied = msgs.map { msg =>
      ...
      // 存储每个符合条件的容器及其下载时间
      val containerDownloadTimes = TrieMap[String, Double]()

      // 遍历所有符合条件的容器
      warmedContainers
        .filter(!inProgressWarmedContainers.values.toSeq.contains(_))
        .filter(_.startsWith(warmedPrefix))
        .foreach { container =>
          val containerId = container.split("/").last
          logging.info(this, s"Evaluating container $containerId for suitability")
          ...
          // 检查该容器是否包含所需的所有表
          val existingTables = containerInfo.map(_.tables).getOrElse(List.empty[String])

          if (tablesNeeded.exists(existingTables.contains)){
            // 将该容器的下载时间添加到 containerDownloadTimes 中
            val downloadTime = TableDownloadPredictor.predictDownloadTime(tablesNeeded, existingTables)
            containerDownloadTimes.put(containerId, downloadTime)
            logging.info(this, s"Predicted download time for container $containerId: $downloadTime ms")
          }
        }
      logging.info(this, s"containerDownloadTimes: $containerDownloadTimes")

      if (containerDownloadTimes.isEmpty) {
        logging.info(this, s"No containers contain any required tables for msg: $msg. Triggering cold start.")
        (msg, None, None)
      } else {
        // 选择下载时间最短的容器
        val optimalContainerId = containerDownloadTimes.toSeq.reduceOption((a, b) => if (a._2 < b._2) a else b).map(_._1)
        logging.info(this, s"choose the container: $optimalContainerId")

        val chosenInvoker = optimalContainerId
          .map(id => warmedContainers.find(_.contains(id)).get.split("/").takeRight(3).apply(0))
          .filter(
            invoker =>
              invokers
        ...
        if (chosenInvoker.nonEmpty && optimalContainerId.nonEmpty) {
          // 热启动
          val containerId = optimalContainerId.get
        ...
        // 不知道代码这里提取dbFile做什么
          ...
          StartInfoManager.createStartInfo(msg.creationId.asString, "wait", "wait", "wait", "warm")// 其实此时containerId也可以确定，但是后面也可以赋值，这里无论是wait还是containerId无所谓
          StartInfoManager.updateContainerIdByCreationId(msg.creationId.asString, containerId)
          (msg, Some(chosenInvoker.get.toInt), Some(warmedContainers.find(_.contains(containerId)).get))
        } else{
          // 冷启动
          (msg, None, None)
        }
