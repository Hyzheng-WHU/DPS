...
case class ContainerDbInfo(containerId: String, dbName: String, tables: List[String])

// 定义 JsonProtocol
object ContainerDbInfoJsonProtocol extends DefaultJsonProtocol {
  // 定义 ContainerDbInfo 的 JsonFormat
  implicit val containerDbInfoFormat: RootJsonFormat[ContainerDbInfo] = jsonFormat3(ContainerDbInfo)
}
...
object ContainerDbInfoManager {
  ...
  // 读取信息文件
  def readDbInfo(): Map[String, ContainerDbInfo] = {
  ...
  // 写入信息文件
  def writeDbInfo(dbInfo: Map[String, ContainerDbInfo]): Unit = {
  ...
  // 更新信息文件
  def updateDbInfo(containerId: String, dbFile: String, tables: List[String]): Unit = {
    val dbInfo = readDbInfo()
    val updatedInfo = dbInfo + (containerId -> ContainerDbInfo(containerId, dbFile, tables))
    writeDbInfo(updatedInfo)
  }

  // 删除容器信息
  def removeContainerInfo(containerId: String): Unit = {
    val dbInfo = readDbInfo()
...
class ContainerManager(jobManagerFactory: ActorRefFactory => ActorRef,
  ...
  protected[container] def extractTableNames(sql: String): List[String] = {
      // 定义正则表达式来匹配 FROM 和 JOIN 后面的表名
      val tableNamePattern: Regex = """(?i)(?:FROM|JOIN)\s+([a-zA-Z_][a-zA-Z0-9_\.]*)""".r

      // 使用正则表达式查找所有匹配的表名
      val tableNames = tableNamePattern.findAllMatchIn(sql).map(_.group(1)).toList
        ...
        if (invokers.isEmpty) {
          ...
          coldCreations.foreach { case (msg, _, _) =>
            ...
            if (!Files.exists(dbPath)) {
              ...
              // 记录容器与数据库信息
              ContainerDbInfoManager.updateDbInfo(msg.creationId.asString, dbFile, tables)
            }
            else {
      ...
      val container = warmedContainers
        ...
        .find { container =>
          if (container.startsWith(warmedPrefix)) {
            ...
            val containerId = container.split("/").takeRight(3).apply(0)            
            ...
            val sqlQueryOpt = msg.args match {
              case Some(jsObj: JsObject) =>
                jsObj.fields.get("sql_query") match {
                  ...
                  case _ => 
                    logging.warn(this, s"sql_query not found or not a JsString in msg.args: $jsObj")
                    None  // 如果不是 JsString，则返回 None
                }
              case None => 
                logging.warn(this, "msg.args is None")
