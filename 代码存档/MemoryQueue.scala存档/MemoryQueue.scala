...
class MemoryQueue(private val etcdClient: EtcdClient,
  ...
  private def handleActivationMessage(msg: ActivationMessage) = {
    ...
    // 提取并存储请求的参数
    // 处理 Option 并将 JsValue 转换为 JsObject
    currentArgs = msg.content match {
      case Some(jsValue: JsObject) => jsValue // 如果是 JsObject 类型，直接赋值
      case Some(jsValue) =>
        logging.warn(this, s"Unexpected content type: ${jsValue.getClass}. Setting empty JsObject.")
        JsObject.empty // 如果不是 JsObject，发出警告并设置为空
      case None =>
        logging.warn(this, s"msg.content is None. Setting empty JsObject.")
        JsObject.empty // 如果没有参数，设置为空
    }
    logging.info(this, s"currentArgs: ${currentArgs}")
