...
class QueueManager(
  ...
  private def handler(bytes: Array[Byte]): Future[Unit] = {
    Future(
      // 调用 ActivationMessage.parse 方法，将字符串解析成 ActivationMessage 对象。
      ActivationMessage
        .parse(new String(bytes, StandardCharsets.UTF_8)))
      .flatMap(Future.fromTry)
      .flatMap { msg =>
        if (isWarmUpAction(msg.action)) {
          // 如果是预热消息：记录日志，直接丢弃消息，不进行激活操作。
          logging.info(
            this,
          ...
          logging.info(
            ...
            msg.transid)
            // 调用 handleActivationMessage(msg) 处理该激活消息
          handleActivationMessage(msg)
        }
        feed ! MessageFeed.Processed

        // 返回一个成功的 Future，以便后续的 flatMap 链可以继续
        Future.successful({})
      }
      .recover {
