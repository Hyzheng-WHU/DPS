...
class DockerClient(dockerHost: Option[String] = None,
  ...
  def run(image: String, args: Seq[String] = Seq.empty[String])(
    ...
    Future {
      blocking {
        ...
        log.info(this, "跳过获取信号量的步骤")
      }
    }.flatMap { _ =>
      // log.info(this, "获取信号量成功, 开始运行docker run")
      ...
      runCmd(
        ...
        .andThen {
          ...
          case _ => 
            val duration = System.currentTimeMillis() - startTime // 计算创建耗时
            log.info(this, s"容器创建完成，耗时: ${duration}ms")
          // log.info(this, "创建完成，释放信号量")
  ...
  def inspectIPAddress(id: ContainerId, network: String)(implicit transid: TransactionId): Future[ContainerAddress] ={
    log.info(this, "开始运行 inspectIPAddress")
    runCmd(
